...
import { MindMapNode, Quiz, LearningPreferences, NodeContent, QuizQuestion, UserAnswer, QuizResult, PreAssessmentAnalysis, ChatMessage, CoachStyle } from '../types';
...
export async function generateChatResponse(history: ChatMessage[], question: string, nodeTitle: string | null, sourceContent: string, style: CoachStyle): Promise<string> {
    return withRetry(async () => {
        ...
        switch (style) {
            case 'concise':
                styleInstruction = 'پاسخ‌های خود را تا حد امکان کوتاه، مختصر و مستقیم ارائه بده.';
                break;
            case 'detailed':
                styleInstruction = 'پاسخ‌های خود را با جزئیات کامل، توضیحات جامع و مثال‌های متعدد ارائه بده.';
                break;
            case 'formal':
                styleInstruction = 'از لحنی کاملاً رسمی، علمی و آکادمیک برای پاسخگویی استفاده کن.';
                break;
            case 'balanced':
            default:
                styleInstruction = 'پاسخ‌های خود را به صورت متعادل، دوستانه و آموزشی ارائه بده.';
        }

        const prompt = `شما یک مربی یادگیری هوشمند، صمیمی و آگاه به نام "مربی هوشمند" هستید. وظیفه شما کمک به کاربر برای درک بهتر مطالب درسی است. ${styleInstruction} بر اساس متن درس و تاریخچه گفتگو به سوال کاربر پاسخ دهید. پاسخ‌های خود را به صورت Markdown ارائه دهید.
